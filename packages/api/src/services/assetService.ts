...
export class AssetService {
  ...
  /**
   * Batch-resolve public CDN (`cloud.oxy.so`) URLs for many avatar file ids in a
   * SINGLE Mongo query and ZERO S3 round-trips — the resolver used by the user
   * serializers so a feed/follower list never triggers an N+1 S3-HEAD storm.
   *
   * For each PUBLIC file it derives the URL for the requested `variant` (default
   * `thumb`) by mapping the recorded variant/original key onto the public CDN
   * path. It does NOT probe S3 for object presence: the placement invariant
   * makes that unnecessary — new public uploads are written under `public/`
   * directly, and the one-shot CDN backfill copied every pre-cutover public
   * object (originals + variants) under `public/`. (Single-asset, correctness-
   * critical paths that need an existence check still use {@link getPublicCdnUrl},
   * which keeps its S3 HEAD.)
   *
   * Returns a `Map<fileId, cdnUrl>` containing only ids that resolved to a public
   * CDN URL. Private/unlisted/missing files and ids that are already absolute
   * URLs are omitted, so callers keep their existing behavior for those.
   *
   * @param fileIds Avatar file ids (bare Oxy file ids). Non-id values
   *                (absolute URLs, empties) are ignored.
   * @param variant Variant to resolve (default `thumb`).
   */
  async resolvePublicAvatarUrls(
    fileIds: ReadonlyArray<string>,
    variant: string = 'thumb'
  ): Promise<Map<string, string>> {
    const resolved = new Map<string, string>();

    // Keep only plausible bare file ids; an absolute URL avatar is already final
    // and must not be treated as an id.
    ...
    for (const file of files) {
      const variantKey = file.variants?.find((v) => v.type === variant)?.key;
      const sourceKey = variantKey || file.storageKey;
      if (typeof sourceKey !== 'string' || sourceKey.length === 0) {
        continue;
      }
      // Map the key onto its public CDN path (idempotent for already-`public/`
      // keys, prefixing legacy `content/`/`variants/` keys to their backfilled
      // public counterpart). No S3 HEAD — see method contract.
      resolved.set(file._id.toString(), cdnUrlForStorageKey(applyPublicPrefix(sourceKey)));
    }
