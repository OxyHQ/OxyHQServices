 ...
 * database to ask. This used to read MongoDB once, before the copy, and hold
 * the answer for the whole run. Production Mongo takes writes throughout the
 * cutover, and the count moved under it: 60,673 → 60,843 → 60,847 `users`
 * across one attempt's three passes. A file uploaded inside that window, naming
 * an account created inside that window, was absent from the snapshot and
 * therefore indistinguishable from a file whose owner had been deleted years
 * ago. On the real run the rule was about to remove EIGHT files with living
 * owners; `resolution-overreach` refused the copy.
 *
 * Reading Postgres at the moment the level is copied has none of that:
 *
 * - **It cannot go stale.** It is the same table, in the same transaction
...
export function assertParentsPrecedeChildren(plans: readonly CollectionPlan[]): void {
  ...
  for (const [index, level] of planLevels(plans).entries()) {
    for (const plan of level) for (const table of planTables(plan)) levelOf.set(tableName(table), index);
  }
 ...
 *
 * One statement per table, selecting one indexed column. `users` is 60,847 rows
 * of a 24-character id — a few megabytes, the same order the audit already
 * holds in memory for its own target sets.
 */
export async function loadParentKeys(
  ...
  for (const table of tables) {
    ...
    // `sqlColumnName` takes its `Column` from — see the note on `idColumnOf` in
    // `runner.ts`: under `moduleResolution: nodenext` drizzle ships CJS and ESM
    // declarations of `PgColumn` that are not assignable to each other, so a
    // column taken from the `pg-core` subpath does not typecheck here.
    ...
    // Written as raw SQL over the table and column NAMES rather than through the
    // query builder, because this is generic over a `PgTable` the caller
    // chooses. `sqlColumnName` is what keeps it from asking for the TypeScript
    // property name — the trap `@oxyhq/db`'s casing module exists to close.
