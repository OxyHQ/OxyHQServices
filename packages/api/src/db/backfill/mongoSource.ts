 ...
 *
 * Production Mongo stays live serving six backends throughout the cutover, so a
 ...
 * object as its read methods, one keystroke apart (`findOne` / `findOneAndUpdate`),
 * and nothing would fail if one were called.
 *
 ...
 *
 * The guard is mutation-tested: `__tests__/mongoSource.test.ts` asserts that a
 * write call throws AND that a read call still works, so a broken Proxy cannot
 ...
 * restarts from `_id > checkpoint` rather than re-reading 296,924 files. The
 * order is total for every collection here, including the two whose `_id` is a
 * string rather than an ObjectId (`usercredits`, `linkpreviews`).
 */
 ...
 *
 * `mongodb` resolves to **v4.17.2** at this workspace's root — hoisted there by
 * `gridfs-stream` — while mongoose 8 uses its own nested `mongodb@6.20.0`. A
 * direct `import type { Db } from 'mongodb'` therefore types the v6 objects
 * this file actually handles against the v4 declarations, which compiles
 * (the shapes are similar enough) and is wrong. Going through `mongoose.mongo`
 * resolves the same copy the runtime values come from, by construction.
 */
 ...
 * `Filter<Document>` REFUSES `{_id: {$gt: someString}}` — and three source
 * collections key on strings: `usercredits` on a userId, `linkpreviews` on a
 * SHA-256, plus every legacy shape. Declaring the union here is the same fact
 * {@link Checkpoint} carries at runtime, stated once at the type level so the
 * resume filter is expressible without a cast.
 *
 * This error only appeared once the driver types were taken from
 * `mongoose.mongo`; against the v4 declarations hoisted at the workspace root
 * it compiled silently.
 */
 ...
 * without going through mongoose — `jest.setup.cjs` mocks mongoose wholesale
 * for the whole suite, and a fake in-memory source would mean the audits were
 * testing an invented `$group`/`$toLower`/`distinct` rather than Mongo's.
 *
 ...
 * through it — the models are all bound to the default connection.
 */
 ...
 *
 * The type is stored, never inferred. `usercredits._id` is a 24-character hex
 * string that is a genuine `String` in BSON, while every ObjectId collection's
 * `_id` renders as the same 24 hex characters — so a "looks like hex ⇒
 * ObjectId" rule gets `usercredits` wrong, and `{$gt: ObjectId(...)}` against a
 * string `_id` matches NOTHING in Mongo. Silently. A resumed run would report
 * "0 documents remaining" and be believed.
 *
 * Two other collections key on strings that are not 24 hex characters
 * (`linkpreviews` on a SHA-256, `federation_keypairs` on an ObjectId), which is
 * exactly the mix that makes inference unsafe.
 */
