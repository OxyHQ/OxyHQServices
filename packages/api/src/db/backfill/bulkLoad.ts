 ...
 * reads without planning anything. On this dataset the difference is the
 * difference between minutes and a long evening.
 *
 ...
 * JavaScript array is either a Postgres array literal (`text[]`) or a JSON
 * array (`jsonb`), and a plain object is either `jsonb` or nothing valid at
 * all — the JS value cannot tell them apart, so the column's declared type is
 * the authority.
 */
export function encodeCopyValue(value: unknown, sqlType: string): string {
  if (value === null || value === undefined) return '\\N';

  const isArrayColumn = sqlType.endsWith('[]');
  const isJsonColumn = sqlType === 'jsonb' || sqlType === 'json';

  if (Buffer.isBuffer(value)) return escapeCopyText(`\\x${value.toString('hex')}`);
  ...
  throw new Error(
    `Cannot encode ${typeof value} for a COPY into a ${sqlType} column; the ` +
      'backfill only produces null, boolean, number, string, Date, Buffer, ' +
      'array and plain-object values'
  );
}
 ...
 * takes drizzle's root-resolved `Column`, and under `moduleResolution: nodenext`
 * the two subpaths declare structurally incompatible versions of that type.
 */
 ...
 * knows nothing about.
 *
 * `generatedId()` (`schema/columns.ts`) is a `$defaultFn`, not a database
 * `DEFAULT` — deliberately, because Postgres 17 has no native `uuidv7()`. So
 * `db.insert(t).values({…})` gets an id and `COPY t (…) FROM STDIN` does not:
 * the column is simply absent from the copy and the row fails `not-null`. That
 * is not a theoretical difference — it fired on
 * `moderation_policy_severity_rules` the first time this loader ran.
 *
 ...
 *
 * Idempotence survives because every table that reaches this WITHOUT an
 * explicit id carries a unique constraint over its natural key
 * (`(message_id, kind, ord)`, `(policy_id, severity)`,
 * `(device_session_id, account_id)`, …), so a re-run conflicts there instead of
 * on the id. The one child table with no such constraint — `file_variants` —
 * supplies a DERIVED id from its transform for exactly this reason.
 */
...
export async function copyRowsInto(
  ...
  for (const group of groupByColumnSet(applyDefaultFns(table, rows))) {
    const columns = group.properties.map((property) => {
      ...
      if (!info) {
        throw new Error(
          `${tableName} has no column with property name ${JSON.stringify(property)} — ` +
