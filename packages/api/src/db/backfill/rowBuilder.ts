 ...
 * (`ownerUserID`), a dot path copied from Mongo (`'plan.name'`), or a column
 ...
 *
 * ## Three distinct checks, because they catch three distinct mistakes
 *
 * 1. **Unknown key** — the trap above. A key that is not a column property.
 * 2. **Generated column** — `hashed_email`, `search_vector`, `geo`, `s3_key`
 *    are `GENERATED ALWAYS ... STORED`. Postgres rejects a write with SQLSTATE
 ...
 * 3. **Missing required value** — a column that is `NOT NULL` with no default
 *    and no supplied value. Postgres would raise `23502`, naming the column but
 *    not the source document; this names both while the document is in hand.
 * 4. **An `undefined` value for any key.** A transform has exactly two ways to
 *    say "no value": supply `null` (write SQL NULL) or OMIT the key (let the
 *    column default apply). `undefined` means neither of those two things
 *    reliably — for a `NOT NULL DEFAULT` column the two answers differ, and
 *    picking the wrong one is either a `23502` at hour three or a column
 *    silently overwritten with NULL. Forcing the choice at the transform is the
 *    only way it stays reviewable.
 *
 * A `null` supplied for a `NOT NULL` column is a MISSING value, not a present
 * one: that distinction is the whole point of check 3, since Mongo's "field
 * absent" and Mongoose's "field null" both arrive here as `null`/`undefined`.
 *
 * ## Why not just rely on the types
 *
 * A hand-written `values({...})` literal IS type-checked, and the per-collection
 * transforms below are written that way wherever the shape is static. But a
 * backfill also builds rows from Mongo documents whose shape TypeScript cannot
 * know, and `Record<string, unknown>` erases exactly the guarantee that matters.
...
export function tableShape(table: PgTable): TableShape {
  ...
  for (const column of config.columns) {
    ...
    // is exactly what `values()` is keyed by. (It is NOT the SQL name — see the
    // trap documented in `schema/CONVENTIONS.md`.)
...
/** Values accepted for a backfill column. */
 ...
 * @throws {BackfillRowError} On an unknown key, a generated column, or a
