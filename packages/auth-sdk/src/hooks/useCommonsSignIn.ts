  ...
  const claimAndCommit = useCallback(
    async (svc: OxyServices, token: string, runId: number) => {
      ...
      try {
        ...
        // The device-flow claim now persists a rotating refresh-token family
        // (device-first), so the legacy per-apex IdP `fedcm_session` establish
        // hop is gone — a reload restores locally from the persisted store.
      } catch (err) {
