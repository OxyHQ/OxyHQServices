...
var API_BASE_URL = (process.env.OXY_API_URL || "https://api.oxy.so").replace(/\/+$/, "");
var FEDCM_ISSUER = (process.env.FEDCM_ISSUER || "https://auth.oxy.so").replace(/\/+$/, "");
var FEDCM_TOKEN_SECRET = process.env.FEDCM_TOKEN_SECRET || "";
var IS_PRODUCTION = false;
var COOKIE_NAME = "fedcm_session";
var COOKIE_MAX_AGE = 30 * 24 * 60 * 60;
var TOKEN_LIFETIME = 600;
var DIST_DIR = join2(import.meta.dirname, "..", "dist");
function base64url(input) {
  const buf = typeof input === "string" ? Buffer.from(input) : input;
  return buf.toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "");
}
function createHS256JWT(payload) {
  const header = base64url(JSON.stringify({ alg: "HS256", typ: "JWT" }));
  const body = base64url(JSON.stringify(payload));
  const signatureInput = `${header}.${body}`;
  const signature = base64url(createHmac("sha256", FEDCM_TOKEN_SECRET).update(signatureInput).digest());
...
app.get("*", async (c) => {
  try {
    const indexPath = join2(DIST_DIR, "index.html");
