/**
 * Expo Config Plugin: withOxySplashBranding
 *
 * Adds the shared Oxy "from Oxy" branding to the NATIVE OS splash screen — the
 * "Instagram, from Meta" pattern: the APP's own logo is centered by the
 * `expo-splash-screen` plugin, and the Oxy symbol is pinned to the BOTTOM by
 * this plugin so every Oxy Expo app carries the same discreet ecosystem mark.
 *
 ...
 *
 * Options (all optional — defaults are the verified, on-device-correct values):
 *   - androidImage: path to the ANDROID branding PNG, authored at the OS
 *                   branding container's 2.5:1 aspect (see ANDROID note below).
 *                   Defaults to the bundled Oxy asset. Resolved relative to the
 *                   project root if it looks project-relative, otherwise as an
 *                   absolute path.
 *   - iosImage:     path to the iOS branding PNG (the TIGHT square symbol, no
 *                   padding — iOS uses scaleAspectFit and does not stretch).
 *                   Defaults to the bundled Oxy iOS asset.
 *   - imageWidth:   iOS-only display width of the branding image in pt. Defaults
 *                   to 48. (Android sizing is fixed by the OS container — see
 ...
 *        bundled Oxy symbol is kept small + centered inside transparent padding
 *        so the fill is uniform and the visible mark stays discreet & undistorted.
 ...
 *        the bitmap DOWN (sharp), never up.
 *
...
function resolveOptions(options) {
  const opts = options || {};
  return {
    androidImage: opts.androidImage,
    iosImage: opts.iosImage,
    imageWidth: typeof opts.imageWidth === 'number' ? opts.imageWidth : 48,
  };
}
      ...
      if (Math.abs(srcAspect - containerAspect) > 0.02) {
        throw new Error(
          `withOxySplashBranding: Android branding source aspect ${srcAspect.toFixed(
            ...
            'the source PNG must be authored at 2.5:1 (symbol centered in ' +
...
module.exports = function withOxySplashBranding(config, options) {
