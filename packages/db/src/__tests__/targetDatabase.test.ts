 ...
 * itself (the same reasoning `database.test.ts` in this package and the
 * Mention source this module was moved from both apply). This package has no
 * live-Postgres test harness yet — that lands in a later task — so
 * `assertMigrationTarget` is left for integration coverage once one exists,
 * rather than given a mock that would assert nothing real.
 */
