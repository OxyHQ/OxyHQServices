...
// `node_modules/@oxyhq/bloom` (hoisted) and `packages/inbox/node_modules/
...
// the hoisted copy, while inbox code resolves to its own local copy. Each
...
// called from inbox code (inbox bloom), and the splash screen crashes
// with "useTheme must be used within a <BloomThemeProvider>".
//
// We use a custom `resolveRequest` to rewrite the `originModulePath` of
// every `@oxyhq/bloom[/subpath]` import to inbox' own package root, so
// Metro's default resolver picks up the package from inbox' node_modules
...
config.resolver.resolveRequest = (context, moduleName, platform) => {
  ...
  if (moduleName === '@oxyhq/bloom' || moduleName.startsWith('@oxyhq/bloom/')) {
    // Pretend the import originated from inbox' own root, so Metro applies
