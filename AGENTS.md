...
- The SDK's device-first cold boot (`runSessionColdBoot` in `@oxyhq/core`, `packages/core/src/boot/coldBootV2.ts`) owns session restore end to end and NEVER auto-redirects to a login page. It is a two-step chain: `device-secret-mint` (web + native — mint from the persisted `deviceId` + `deviceSecret`) then `shared-key-signin` (native — re-mint from the shared Commons keychain). Apps do not implement local session restore or sign-in screens — the mint client, storage keys, and the re-mint handler/scheduler live once in `@oxyhq/core`.
