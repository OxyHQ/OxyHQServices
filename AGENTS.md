...
- `packages/services/src/ui/oauth/` — `browserAuthTransport.ts` (`startWebOAuthSignIn`, popup-vs-redirect entry point), `completeOAuthCode.ts` (the one code→session completion path both transports share), `oauthPopup.ts` (popup lifecycle), `legacyRedirectLanes.ts` (`allowsAutomaticIdpRedirect`) — issue #691 Phase 2/7a
...
- Commons' approval screen (`packages/commons/components/commons-signin/approval-request.tsx`) is ONE confirmation: "Confirm identity" opens the device biometric/passcode prompt DIRECTLY — no intermediate "Continue" step. The only other answer is "This wasn't me"; a plain dismiss answers nothing. `POST /auth/session/deny/:authorizeCode` accepts an optional `reason` from a CLOSED set — `AUTH_SESSION_DENY_REASONS = ['declined', 'not_me']` — anything else 400s before the handler runs.
