...
- **This is a Cloudflare Pages Functions DIRECTORY (`functions/`, file-based routing), NOT an advanced-mode single `dist/_worker.js`.** An earlier attempt (commit `8c44c6fb`/#544) tried to fix a broken deploy by keeping advanced mode and adding an execution-assertion script; that did not work — CF Pages was not detecting/invoking the advanced-mode worker on this project AT ALL (reproduced even on the direct `<hash>.oxy-auth.pages.dev` deployment URL, ruling out edge/routing causes). The fix that actually worked (commit `1141ddb7`/#545) was migrating to the Functions-directory shape CF reliably detects. The deploy workflow verifies compilation with `bunx wrangler@4 pages functions build functions --outfile ...` (the same command CF runs internally) before deploying — a green compile is a strong signal the route will register.
- **⚠️ As of this writing the auth.oxy.so deploy pipeline itself is BROKEN** (separate from the Functions-directory fix above): `cloudflare/wrangler-action@v3` invoked with `workingDirectory: packages/auth` runs its wrangler install via `npx`, which triggers npm's Arborist and fails with `npm error code EOVERRIDE` against the repo-root `overrides["@oxyhq/bloom"]` (a legitimate, monorepo-wide, do-not-remove override) because `packages/auth`'s direct `@oxyhq/bloom` dep conflicts with it under npm's (not bun's) override-conflict check. Until this is fixed (replace the `wrangler-action` step with a direct `bunx wrangler@4 pages deploy dist ...` `run:` step, scoped to the auth job only, so npm is never invoked), the auth.oxy.so deploy fails and prod keeps serving whatever was last successfully deployed. **Verify before trusting this section**: `curl -s https://auth.oxy.so/api/device-accounts` should return JSON, not `<!doctype html>`; check `gh run list --workflow=deploy-cloudflare.yml` for the latest `deploy-auth` job conclusion.
