...
|---------|---------|-------|
| `@oxyhq/core` | **3.4.1** | 3.4.1: identity-scoped HttpService GET response cache (cache keys + dedupe key include per-identity tag; `clearTokens()` clears cache on logout); fixes "Who to follow" anon-cache-served-to-authed-refetch bug. 3.3.0: `reputation` mixin (14 methods) + 20 types; karma mixin + `KarmaRule`/`KarmaHistory`/`KarmaAwardRequest` types removed — BREAKING (but shipped in 3.x range, not bumped to 4.0.0). 3.2.0: additive — `PublicApplication` + `getPublicApplication(clientId)`; `OxyConfig.clientId`; `AuthManager.getAccessToken()` falls back to `_lastKnownAccessToken`; `workspaces` mixin; `getApplications(workspaceId?)`; invite inputs `{ usernameOrEmail, role }`. 3.1.0: credential rotation grace. 3.0.0 BREAKING: removed `developer` mixin; added `applications` mixin. **GOTCHA: 3.3.0 and 3.4.0 are BROKEN for external consumers** (declared `workspace:*` dep on unpublished `@oxyhq/contracts`). Pin to ≥3.4.1. 2.4.1: FedCM silent hang fix. 2.3.0: SSO helpers single source of truth. |
...
| `@oxyhq/services` | **10.2.0 (working tree has unpublished Trust screen changes)** | Working tree: Karma*→Trust* screen rename; BREAKING `RouteName`: removed `KarmaCenter|KarmaLeaderboard|KarmaRewards|KarmaRules|AboutKarma|KarmaFAQ`; added `TrustCenter|TrustLeaderboard|TrustRewards|TrustRules|AboutTrust|TrustFAQ`. Consumers calling `showBottomSheet('Karma...')` MUST migrate. 10.2.0: additive (details TBD). 10.1.0: native device-flow sign-in fix. 10.0.0 BREAKING: `appName` prop REMOVED from `OxyProvider`; cross-app device sign-in requires `clientId`; peer `@oxyhq/core ^3.2.0`. 9.0.1: `silentGuardKey.ts` RN-safe. 8.6.1: `COLD_BOOT_OVERALL_DEADLINE`. 8.5.0: `isAuthResolved`. 8.4.0: core SSO helpers. 8.0.0 BREAKING: `@tanstack/*` → peerDeps; `RouteName` ManageAccount. |
...
**Consumer apps on latest (2026-06-16):** Mention on `@oxyhq/core 3.4.1` (commit 09551664) + `@oxyhq/services 10.2.0`. Homiio, Alia, Allo, Syra, TNP, OxyHQServices apps bumped to `@oxyhq/bloom 0.7.7` + `@oxyhq/services 10.2.0` (where used) + `@oxyhq/core 3.2.0` (pre-3.4.1 — update as each app migrates). Removing `oxy.developer.*` calls required before upgrading to core 3.0.0+.
