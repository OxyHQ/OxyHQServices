...
- **A module that names an OPTIONAL peer must never be reachable from the root barrel.** `tsc` resolves the specifier of an `import()` even when the call is lazy and wrapped in try/catch, so a barrel re-export drags the specifier into EVERY consumer's type graph and turns an optional peer into a hard install requirement (`TS2307`, plus `TS7006` cascades where parameters lose contextual typing). This is resolver-asymmetric and therefore easy to ship unnoticed: web/Vite consumers resolve the package through `lib/**/*.d.ts` and `skipLibCheck: true` hides it, while Metro/RN consumers resolve the `react-native` condition (published `src/`) and fail. Metro itself is fine — an unresolvable DYNAMIC `import()` bundles cleanly and the `catch` handles it at runtime; only a STATIC import fails the Metro build. **Which of the two fixes applies is decided by whether the module has an internal consumer, NOT by preference:** a module with NO internal consumer is opt-in and gets its own export subpath (the push adapter — `@oxyhq/services/notifications`, `packages/services/src/notifications/deviceNotifications.ts`, kept out of the barrel by `packages/services/__tests__/notifications/barrelIsolation.test.ts`); a module that backs a CORE screen is reclassified as a REQUIRED peer, because hiding core UI behind a subpath would be worse than requiring the install (`expo-image-picker`, `expo-image-manipulator`, `expo-haptics`, `expo-document-picker`, `expo-web-browser`). Both directions are held by `packages/services/__tests__/packaging/peerDeclarations.test.ts`, which walks the real module graph in TWO variants — `tsc`'s (imports only) and Metro's (also `require()`, which `ui/navigation/routes.ts` uses for its lazy screen table). A specifier reachable only through `require()` does not break `tsc` today but still backs a screen the SDK's router can open, so it must be required too. Do NOT "fix" this class with an ambient `declare module 'expo-x'` in `src/types/`: an ambient declaration SHADOWS the real package for the whole consumer program, so an app that DID install the module silently loses its real types — and in-package it hides exactly the missing declaration you are looking for (the `expo-haptics` / `expo-document-picker` stubs did that, and were deleted). The surviving `src/types/expo-*.d.ts` stubs serve modules loaded through a runtime-computed specifier (`const M = 'expo-secure-store'; await import(M)`), which is invisible to both `tsc` and Metro by design.
- **A package's own `devDependencies` must cover every module its `src/` names — a type-check that passes because a sibling workspace package hoisted the module into the root `node_modules` is not a gate.** `packages/services` type-checked against `@oxyhq/bloom`, `expo-image-picker`, `expo-image-manipulator`, `expo-web-browser` and `@testing-library/react` purely by that accident; installed from its own manifest alone it produced 332 errors. Verify by installing the package's `dependencies` + `devDependencies` into an empty directory and running `tsc` there — and set `types: []` in that harness, because one broken implicit `@types` entry aborts `tsc` before it reports a single source error, which reads as a clean pass.
...
- **Removed unused deps**: `@radix-ui/react-tabs`, `react-responsive`, `@lottiefiles/dotlottie-react-native`, `expo-symbols`. KEEP `expo-document-picker` + `expo-image-manipulator` (lazy-loaded, but REQUIRED peers of `@oxyhq/services` — they back its FileManagement and AvatarCrop screens) and `@lottiefiles/dotlottie-react` (hard-required by web lottie export).
