...
|---------|---------|-------|
| `@oxyhq/core` | **3.4.18** | Current target. Includes `@oxyhq/core/server` (`createOxyAuthMiddleware`, `createOptionalOxyAuth`, `createOxyRateLimit`, `requireOxyAuth`, `getRequiredOxyUserId`) plus linked backend clients. 3.4.18: preflight refresh fallback keeps a still-valid near-expiry bearer token instead of dropping to cookie/CSRF mode; this prevents linked RP APIs from probing app-local `/csrf-token` while the token is still usable. 3.4.7: `OxyServices.createLinkedClient({ baseURL })`. 3.4.5: `getSsoCallbackBootstrapScript()` + `ssoCallbackBootstrapKey()`. 3.4.1: identity-scoped HttpService GET response cache. 3.3.0: `reputation` mixin; karma mixin/types removed. 3.2.0: `clientId`, workspaces, application scoping. **GOTCHA: 3.3.0 and 3.4.0 are broken for external consumers** (`workspace:*` dep on unpublished contracts). Pin to ≥3.4.18 for current apps. |
