...
Monorepo (`@oxyhq/sdk`) using Bun workspaces + Turbo. Build order matters: `contracts` -> `core` -> `auth` -> `services` -> rest.
...
**NOTE: `DeveloperApp` model is DELETED. Service tokens now use `Application` + `ApplicationCredential` (see main AGENTS.md "Application Model" section).**
...
See the main OxyHQServices AGENTS.md "Published Package Versions" section for the current authoritative versions. This file was a snapshot from 2026-05-30 and those version numbers are stale.
