...
**Consumer apps on latest (2026-06-18):** Mention, Syra, Allo, Homiio, Alia, TNP, website, and OxyHQServices apps target `@oxyhq/core 3.4.5`, `@oxyhq/auth 4.1.1` where used, `@oxyhq/services 10.2.2` where used, and `@oxyhq/bloom 0.8.5` where used. Expo web apps that can receive `/__oxy/sso-callback` inject `getSsoCallbackBootstrapScript()` in `app/+html.tsx`.
