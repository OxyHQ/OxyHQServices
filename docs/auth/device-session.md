...
|--------|------|------|----------|
| POST | `/session/device/token` | `{ deviceId, deviceSecret, accountId? }` | **Zero-cookie mint** — public. Verifies `sha256(deviceSecret)` (constant-time) against the device's `secretHash`, mints a short access token for the active account, and rotates the secret in-use (returns `nextDeviceSecret`; the presented secret stays valid for a 60s grace). `401 invalid_device_secret` on a bad/diverged secret; `401 no_active_session` when the device is known but has no live session (no rotation). Per-device lockout + `rl:session:device-token:` rate limit. **Optional `accountId`** (issue #691 Phase 1) PINS the mint to that one account of the device's set instead of whichever is active — it NEVER mutates `activeAccountId`/`revision` and never broadcasts, and the returned `state` still reports the device's true active account; a non-member account or one whose session is dead answers `401 account_not_on_device` (no rotation), deliberately indistinguishable so a pinned miss can't become an account-existence oracle. Used by `sessionMode: 'identity'` (Commons) — see [SESSION-ARCHITECTURE.md](../SESSION-ARCHITECTURE.md) § Cold boot. |
