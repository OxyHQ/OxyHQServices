...
- **Multi-domain FAPI + cross-domain SSO (Clerk-style CNAME pattern, 2026-06-13)**: any RP CNAMEs `auth.<rp-domain>` → `oxy-auth.pages.dev`; the IdP responds with an issuer matching the RP's apex — `fedcm_session` cookie is first-party in Safari/Firefox. `resolveConfig()` in `packages/auth/server/index.ts:110` derives `fedcmIssuer` from `c.req.url` per-request; `/fedcm.json` is a dynamic handler (NOT a static asset). Live on `auth.oxy.so`, `auth.mention.earth`, `auth.alia.onl`, `auth.homiio.com`. Session restore cold boot order (web): 1. redirect-callback → 2. FedCM silent (Chrome) → 3. first-party iframe `/auth/silent` at `auth.<apex>` (Safari/Firefox) → 4. cookie restore → 5. stored-session bearer. Native: step 5 only. Primitive: `runColdBoot` in `packages/core/src/utils/coldBoot.ts` (pure ordered short-circuit, no module-level state). `autoDetectAuthWebUrl` in `packages/core/src/utils/fapiAutoDetect.ts` (MOVED from auth-sdk) — bails on localhost/IP/IPv6/single-label/multi-part-TLD. New IdP endpoints (`packages/auth/server/index.ts`): `/auth/silent` reads first-party `fedcm_session` cookie, mints Oxy access token via existing FedCM assertion→nonce→exchange pipeline, returns HTML that `postMessage`s `{type:'oxy_silent_auth', session, nonce}` to allow-listed `client_id` origin ONLY (NEVER `targetOrigin='*'`). `/auth/session-check` validates cookie. No `api.<apex>` cookie bridge needed. Consumers (Mention/Homiio/Alia) on `@oxyhq/services 8.2.0` + dropped `authWebUrl=auth.oxy.so` pin so they auto-detect. Mention additionally bumped to `@oxyhq/core 2.1.2` (passes its own OxyServices instance → needs constructor auto-detect).
...
|---------|---------|-------|
| `@oxyhq/core` | **2.1.2** | 2.1.2: OxyServices constructor auto-detects `auth.<apex>` from `window.location` (critical for apps passing their own instance, e.g. Mention). 2.1.1: CrossDomainAuth logs via logger. 2.1.0: `runColdBoot` + `autoDetectAuthWebUrl` moved into core from auth-sdk |
| `@oxyhq/auth` | **3.2.0** | 3.2.0: `WebOxyProvider` consumes core `runColdBoot` + `autoDetect`; `fapiAutoDetect` deleted from auth-sdk; guard key `origin\|baseURL`; init-effect `.catch` hardening. 3.1.0: multi-domain FAPI auto-detect. 3.0.0: major bump |
| `@oxyhq/services` | **8.2.0** | 8.2.0: FedCM-first cross-domain cold boot via `runColdBoot`; iframe `/auth/silent` fallback (Safari/Firefox); constructor/provider auto-detect; native = stored-session only; IdP poller force-logs-out only on same-site IdP. 8.1.1: removed legacy `@react-navigation/native` peer. 8.0.1: fixed `expo-crypto` shim |
...
- **`/auth/silent`** (NEW): reads `fedcm_session` cookie, mints Oxy access token via FedCM pipeline server-side, returns HTML that `postMessage`s session to allow-listed `client_id` origin. Enables Safari/Firefox cross-domain restore.
- **`/auth/session-check`** (NEW): validates `fedcm_session` cookie; used by RP to check IdP session before attempting silent flow.
