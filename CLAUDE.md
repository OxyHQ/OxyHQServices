...
- **Multi-domain FAPI (Clerk-style CNAME pattern, 2026-06-13)**: any RP can CNAME `auth.<rp-domain>` to `oxy-auth.pages.dev` and the IdP responds with an issuer matching the RP's apex — the browser treats every endpoint as same-site with the RP, so the `fedcm_session` cookie is first-party in Safari/Firefox (no third-party cookie gating). `resolveConfig()` in `packages/auth/server/index.ts:110` derives `fedcmIssuer` from `c.req.url` per-request; `/fedcm.json` is a **dynamic handler** (NOT a static asset under `public/`) so icon URLs are absolute to the request host. Live in prod on `auth.oxy.so`, `auth.mention.earth`, `auth.alia.onl`, `auth.homiio.com`. On the client, `WebOxyProvider` auto-detects `https://auth.<eTLD+1>` via `utils/fapiAutoDetect.ts` (bails out on localhost/IP/single-label; short-circuits when already on `auth.*`); explicit `authWebUrl` prop still wins.
...
| `@oxyhq/auth` | **3.1.0** | 3.1.0: multi-domain FAPI auto-detect (`utils/fapiAutoDetect.ts`) + FedCM-first cold boot order in `WebOxyProvider`. 3.0.0: major bump |
